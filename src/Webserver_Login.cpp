...
{
    LOG_DP("Login attemp from " PRsIP, PRIPVal(request->client()->remoteIP()));
    ...
    if (!request->authenticate(Config.auth_user.c_str(), Config.auth_passwd.c_str())) {
        LOG_EP("Invalid login attemp from " PRsIP, PRIPVal(request->client()->remoteIP()));
        return request->requestAuthentication(Config.DeviceName.c_str());
    }
    request->send(200, F("text/plain"), F("Success"));
    LOG_IP("Login from " PRsIP, PRIPVal(request->client()->remoteIP()));
}
